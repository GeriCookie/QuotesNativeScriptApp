...
var updateController = function(User, Quote, Update) {
    ...
    var getFollowing = function(req, res) {
      var friendsIds = req.user.friends.map(friend => friend.userId.toString());

      var page = req.query.page || 1;

      function isInFriends(user) {
        return friendsIds.indexOf(user.userId.toString()) >= 0;
      }
      ...
      Update.find({})
        ...
        .exec(function(err, updates) {
          ...
          updates = updates.filter(update => isInFriends(update.user))
